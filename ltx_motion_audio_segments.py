...
def _read_waveform_peaks(audio_file, bins=1200):
    ...
    if extension == ".wav":
        ...
        if ffmpeg_path is None:
            raise ValueError("Waveform preview for MP3 and other compressed formats requires ffmpeg")
        ...
        if result.returncode != 0:
            ...
            raise ValueError(f"Failed to decode audio waveform: {stderr or 'ffmpeg returned a non-zero exit code'}")
...
def _select_keyframed_storyboard_segment(audio, items, keyframes, segment_index=0):
    if not items:
        raise ValueError("At least one storyboard image is required")
    ...
    segment_end_seconds = min(natural_end_seconds, total_duration)
    if segment_end_seconds <= start_seconds:
        segment_end_seconds = min(total_duration, start_seconds + max(1.0 / sample_rate, 0.1))

    start_frame = int(math.floor(start_seconds * sample_rate))
    end_frame = int(math.ceil(segment_end_seconds * sample_rate))
    selected_audio = _slice_audio(audio, start_frame, end_frame)
    selected_item = items[min(current_segment, len(items) - 1)]
    segment_duration_seconds = max(0.1, segment_end_seconds - start_seconds)
    debug_text = (
...
class LTXMotionWaveformStoryboardSelector:
    ...
    def select_segment(
        ...
        **kwargs,
        ...
        debug_text = (
            f"{debug_text} | dynamic_images={connected_image_indexes} | "
...
class LTXMotionStoryboardPromptSelector:
    ...
    def VALIDATE_INPUTS(cls, current_segment=0, prompt_count=1, segment_count=0, **_kwargs):
        try:
            resolved_current_segment = cls._coerce_int(current_segment, 0)
            resolved_segment_count = cls._coerce_int(segment_count, 0)
            resolved_prompt_count = cls._coerce_int(prompt_count, 1)
            count_source = resolved_segment_count if resolved_segment_count > 0 else resolved_prompt_count
            min(max(count_source, 1), MAX_STORYBOARD_SLOTS)
            max(resolved_current_segment, 0)
        except Exception as exc:
            return str(exc)
        return True
    ...
    def select_prompt(
        ...
        **kwargs,
        ...
        resolved_current_segment = self._coerce_int(current_segment, 0)
        resolved_segment_count = self._coerce_int(segment_count, 0)
        resolved_prompt_count = self._coerce_int(prompt_count, 1)
        active_prompt_count = resolved_segment_count if resolved_segment_count > 0 else resolved_prompt_count
        active_prompt_count = min(max(active_prompt_count, 1), MAX_STORYBOARD_SLOTS)
        selected_index = max(0, min(resolved_current_segment, active_prompt_count - 1))
