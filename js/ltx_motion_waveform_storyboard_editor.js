...
function getEffectiveAudioFilename(node) {
    const connectedAudioFile = getConnectedAudioFilename(node);
    if (connectedAudioFile) {
        return connectedAudioFile;
    }
    const audioFileWidget = getWidget(node, "audio_file");
    const widgetAudioFile = String(audioFileWidget?.value || "").trim();
    return widgetAudioFile || null;
}
...
function buildEditor(node) {
    ...
    const loadWaveform = async (force = false) => {
        const audioFileWidget = getWidget(node, "audio_file");
        const connectedAudioFile = getConnectedAudioFilename(node);
        if (connectedAudioFile && audioFileWidget) {
            audioFileWidget.value = connectedAudioFile;
        }

        const audioFile = getEffectiveAudioFilename(node);
        if (!force && state.currentAudioFile === audioFile) {
            return;
        }
...
app.registerExtension({
    ...
    async beforeRegisterNodeDef(nodeType) {
        ...
        nodeType.prototype.onDrawForeground = function () {
            ...
            this.__ltxWaveformEditor?.loadWaveform?.(false);
            return result;
        };
    },
